...
def load_chaperones_graph(graph_path, disease_tissue, disease_info):
...
def eval_weight_predictor(gdf, classifier, clf_params={}):
    X_train = gdf.copy()
    ...
    scores = cross_val_score(weight_clf, list(X_train["features"]) , y_train, scoring="neg_mean_squared_error", cv=10)
    return scores.mean()
    
def get_weight_predictor(gdf, classifier, clf_params={}):
...
def get_emb_clf(gdf, classifier, clf_params={}):
